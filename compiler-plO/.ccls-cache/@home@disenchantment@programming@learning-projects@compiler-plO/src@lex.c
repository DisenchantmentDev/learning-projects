 ...
 * - lex(): the actual lexer. Skips over white space, checks for ident, number,
 * and comment outputs the token into *raw which is read by the parser.
 */

static void comment(void) // method for lexing past comments
{
  /*
   * Only currently has functionality for { ... } comments. pl0 also should
   * support comments with the {* ... *} format. TODO later. Comments can be
   * nested, and you can mix and match opening and closing comment characters.
   */
  int ch;

  while ((ch = *raw++) != '}') { // loops to check if the next character is an
                                 // ending comment character
    if (ch == '\0') // if we hit the end of the file, a comment was not properly
                    // closed
...
static int ident(void) {
  ...
  /* Table that checks if our token is a reserved word. If not, then it's an
   * identifier*/
...
int lex(void) // lexer function. Just a big switch/case checking the token of
              // each character
{
