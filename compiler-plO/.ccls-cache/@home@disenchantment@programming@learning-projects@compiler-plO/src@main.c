...
error (const char *fmt, ...)
{
    ...
    (void)fprintf (stderr,
                   "pl0c error: %lu: ", line); // print the line w/ error
...
readin (char *file)
{
    ...
    if (strrchr (file, '.')
        == NULL) // strrchr() loactes the last occurance of a
        // specific character in a string.
        // In this case, checking if there is a single occurance of '.' in the
        // file name
    ...
    if ((fd = open (file, O_RDONLY))
        == -1) // checks to see if opening the given file descripter fails. If
               // it
    ...
    while ((type = lex ()) != 0)
        {
            ...
            switch (type)
                ...
                case TOK_ODD:
                    (void)fprintf (
                        stdout, "%s",
                        token); // standard formatted output to stream
                    break;
                case TOK_DOT:
                case TOK_EQUAL:
                case TOK_COMMA:
                ...
                case TOK_LPAREN:
                case TOK_RPAREN:
                    (void)fputc (type, stdout); // fputc for putting a single
                                                // character into the stream
                    break;
                case TOK_ASSIGN:
                    (void)fputs (
                        ":=",
                        stdout); // fputs for putting a null-terminated string
                                 // into the stream
                }
            (void)fputc ('\n', stdout);
        }
