...
comment (void) // method for lexing past comments
{
     ...
     * nested, and you can mix and match opening and closing comment
     * characters.
     */
    int ch;

    while ((ch = *raw++) != '}')
        {                   // loops to check if the next character is an
                            // ending comment character
            if (ch == '\0') // if we hit the end of the file, a comment was not
                            // properly closed
...
int
lex (void) // lexer function. Just a big switch/case checking the token of
           // each character
{
