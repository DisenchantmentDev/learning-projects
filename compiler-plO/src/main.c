// Complier for PL/O, a simplified version of Pascal, designed by the same
...
void error(const char *fmt, ...) {
  va_list ap; // va_list is a struct for error function with variable # of
              // inputs

  (void)fprintf(stderr, "pl0c error: %lu: ", line); // print the line w/ error
...
static void readin(char *file) {
  ...
  struct stat st; // stat struct describes a struct which stores information
                  // about an open file.
  // Specifics found here:
  // https://pubs.opengroup.org/onlinepubs/009695299/basedefs/sys/stat.h.html

  if (strrchr(file, '.') == NULL) // strrchr() loactes the last occurance of a
                                  // specific character in a string.
    // In this case, checking if there is a single occurance of '.' in the file
    // name
    error("file must end in .pl0");

  if (!!strcmp(strrchr(file, '.'),
               ".pl0")) // strcmp compares two strings. Checking to see if file
                        // properly ends in .pl0
    error("file must end in .pl0");

  if ((fd = open(file, O_RDONLY)) ==
      -1) // checks to see if opening the given file descripter fails. If it
          // does then error, otherwise stores the result in fd
    error("couldn't open %s", file);

  if (fstat(fd, &st) == -1) // get the stat of file.
    error("couldn't get file size of given file");

  if ((raw = malloc(st.st_size + 1)) ==
      NULL) // attempt to malloc required memory based on the size of file
            // stored in st
    error("mallco failed");
...
static void parse_print(void) {
  ...
  while ((type = lex()) != 0) {
    ...
    case TOK_RPAREN:
      (void)fputc(
          type, stdout); // fputc for putting a single character into the stream
      break;
    case TOK_ASSIGN:
      (void)fputs(
          ":=",
          stdout); // fputs for putting a null-terminated string into the stream
    }
    (void)fputc('\n', stdout);
  }
}
